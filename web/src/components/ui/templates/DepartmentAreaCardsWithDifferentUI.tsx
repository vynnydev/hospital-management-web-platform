...
const departmentIcons: Record<string, JSX.Element> = {
    uti: <FaHeart size={12} />,
    enfermaria: <FaUserNurse size={12} />,
    pediatria: <FaChild size={12} />,
    cardiologia: <FaProcedures size={12} />,
    placeholder: <FaHospital size={12} />
};

interface IDepartmentAreaCardsWithDifferentUIProps {
...
export const DepartmentAreaCardsWithDifferentUI: React.FC<IDepartmentAreaCardsWithDifferentUIProps> = ({ 
    departments, 
    onClick, 
    selectedArea,
    loading,
    ...
    const abbreviateDepartment = (area: string) => {
        console.log("Departamento:", area)
        ...
        <div className="relative">
            <Slider {...settings} className="pl-12">
                {uniqueDepartments.map(({ area, count, capacity }) => {
                    ...
                    return (
                        <div key={area} className="inline-block mr-4">
                            <motion.div
                                whileHover={{ scale: 1.03 }}
                                className={`relative p-6 rounded-2xl cursor-pointer transition-all duration-300 
                                        shadow-lg bg-gradient-to-br ${colors.bg} w-[220px] h-[120px]
                            ...
                            >
                                ...
                                <div className="relative z-10 flex flex-row space-x-6 items-center h-full">
                                    <div className="flex flex-col items-center space-y-4">
                                        <h3 className="text-sm font-bold text-white uppercase tracking-wide">
                                            {abbreviateDepartment(area)}
                                        </h3>
