      ...
      <CardContent>
        {!policies || policies.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 text-center">
            ...
            <p className="text-sm text-gray-500 max-w-sm mt-2 mb-4">
              Políticas de acesso baseado em função ajudam a controlar quem pode acessar quais recursos do sistema.
            </p>
            <Button onClick={handleCreatePolicy}>Criar Primeira Política</Button>
          </div>
        ) : (
        ...
        {editingPolicy && (
          <Dialog open={!!editingPolicy} onOpenChange={(open) => !open && setEditingPolicy(null)}>
            <DialogContent className="max-w-2xl">
              <DialogHeader>
                <DialogTitle>{isCreatingPolicy ? 'Nova Política de Acesso' : 'Editar Política de Acesso'}</DialogTitle>
                <DialogDescription>
              ...
              <div className="space-y-4 mt-2">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="policy-role">Função</Label>
                    <Select 
                ...
                <div className="space-y-3">
                  ...
                  {editingPolicy.permissions.length === 0 ? (
                    <div className="text-center py-6 border rounded-md bg-gray-50">
                      <p className="text-sm text-gray-500">Nenhuma permissão configurada</p>
                      <Button 
                        variant="link" 
                        onClick={handleAddPermission}
                        className="mt-1"
                    ...
                    <div className="space-y-3">
                      {editingPolicy.permissions.map((permission, index) => (
                        <div key={index} className="border rounded-md p-3 space-y-3">
                          <div className="flex justify-between items-center">
                            <div className="font-medium">Permissão {index + 1}</div>
                            <Button 
                              variant="ghost" 
                              size="icon"
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div className="space-y-2">
                              <Label htmlFor={`resource-${index}`}>Recurso</Label>
                              <Select 
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
                              {allPermissionActions.map((action) => (
                                <div key={action} className="flex items-center space-x-2 p-2 border rounded-md">
                                  <Checkbox
                                  ...
                                  />
                                  <Label htmlFor={`action-${index}-${action}`}>{getActionLabel(action)}</Label>
                                </div>
                              ))}
                            </div>
                          </div>
